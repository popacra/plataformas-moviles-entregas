...
function enumerarLista(listaDePalabras) {
    return listaDePalabras.map((elemento,indice) =>{
        let mayuscula = elemento.charAt(0).toUpperCase() + elemento.slice(1);
        let organizar = indice + 1 + " " + mayuscula;
        organizar = organizar + ",";
        if(indice === listaDePalabras.length - 1){
            organizar = "Y" + " " + organizar.slice(0,-1) + "." ; 
        }
        return organizar; 
    }
)
